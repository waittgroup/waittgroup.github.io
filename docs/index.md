# Waitt Group Wiki

## Our mission

The Waitt Group works at the intersection of computational chemistry,
bioinformatics, and taste science. Our objective is to understand and
predict taste at the molecular level — combining QM-derived molecular
descriptors, receptor sequence analysis, and machine learning to model
how molecular structure and receptor biology drive taste perception,
using ORCA / OSPool-HTC computational workflows to scale it all.

## About this wiki

This site is the group's SOP hub and onboarding reference: a
searchable, single source of truth for the workflows, tools, and
conventions we use day to day, so new members can get productive
without needing to ask around for tribal knowledge.

## Start here

- New to the group? Read the [New Member Guide](new-member-guide.md).
- Looking for the team? See [People](people.md).
- Want to know what we work on? See [Research](research.md).
- Interested in joining? See [Join Us](join.md).
- Need a specific workflow or protocol? Browse the SOPs section in the
  nav, starting with [ORCA on OSPool (HTCondor)](sops/orca-ospool-workflow.md).

## Site details

This wiki is built with [MkDocs Material](https://squidfunk.github.io/mkdocs-material/)
and deployed via GitHub Actions. Pages are plain Markdown under `docs/`.
See the repo's `CLAUDE.md` for conventions if you're contributing.
